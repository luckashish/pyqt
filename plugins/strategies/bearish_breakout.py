"""
Bearish Breakout Expert Advisor.
Sells on specific candlestick pattern:
- Current candle high > previous candle high
- Current candle low < previous candle low
- Current candle close < previous candle close
Entry at previous-to-previous candle low (candle[2].low)
Stop loss at previous candle high with buffer
"""
import pandas as pd
import numpy as np
...
class BearishBreakoutEA(ExpertAdvisor):
    """
    Bearish Breakout Expert Advisor.
    
    Strategy:
    - Detects when current candle makes higher high but lower low and lower close
    - This indicates weakness after a bounce
    - Enters at previous-to-previous candle low (breakdown)
    - Stop loss at previous candle high with buffer
    """
    ...
    def __init__(self):
        ...
        # EA Info
        self.name = "Bearish Breakout EA"
        self.version = "1.0"
        self.author = "System"
        self.description = "Price action pattern EA - Bearish breakdown after bounce"
    ...
    def _load_initial_candles(self):
        """Load initial candles from feed manager or wait for live candles."""
        try:
            ...
            if candles:
                ...
                # No candles in feed manager - will wait for live candles to build up
                logger.warning(
                    f"{self.name}: No historical candles available. "
                    f"EA will start detecting patterns after {3} candles close. "
                    f"This may take {3} minutes on {self.config.timeframe} timeframe."
                )
                self.candles_buffer = []
    ...
    def _check_pattern(self, current_price: float):
        """
        Check for bearish breakout pattern.
        
        Pattern:
        - candle[0] (current): high > candle[1].high, low < candle[1].low, close < candle[1].close
        - Entry: candle[2].low
        - SL: candle[1].high + buffer
        """
        if len(self.candles_buffer) < 3:
            return
        ...
        lower_close = candle_0.close < candle_1.close
        
        if higher_high and lower_low and lower_close:
            # Pattern detected!
            logger.info(
            ...
            self._generate_sell_signal(candle_0, candle_1, candle_2, current_price)
    ...
    def _generate_sell_signal(
        ...
        """
        Generate sell signal with entry and stop loss.
        ...
        """
        # Apply filters
        if not self._check_filters(current_price, is_buy=False):
            return
        ...
        entry_price = prev_prev_candle.low  # Enter at candle[2] low (breakdown)
        
        # Stop loss: previous candle high + buffer
        sl_buffer_pips = self.config.parameters.get('sl_buffer_pips', 5)
        pip_value = 0.0001  # For 4-digit quote
        
        sl_price = prev_candle.high + (sl_buffer_pips * pip_value)
        
        # Calculate take profit based on risk:reward
        risk_pips = abs(sl_price - entry_price) * 10000
        tp_pips = risk_pips * 2  # 1:2 risk:reward
        tp_price = entry_price - (tp_pips * pip_value)
        
        # Override with config TP if specified
        if self.config.take_profit_pips > 0:
            tp_price = risk_manager.calculate_take_profit(entry_price, False, self.config.take_profit_pips)
        ...
        self.generate_signal(
            ...
            reason=(
                f"Bearish Breakout Pattern: "
                f"Higher High ({current_candle.high:.2f} > {prev_candle.high:.2f}), "
                f"Lower Low ({current_candle.low:.2f} < {prev_candle.low:.2f}), "
                f"Lower Close ({current_candle.close:.2f} < {prev_candle.close:.2f}). "
                f"Entry at candle[2] low: {entry_price:.2f}"
            ),
            confidence=0.75
        )
    ...
    def _check_filters(self, price: float, is_buy: bool) -> bool:
        ...
        Args:
            price: Current price
            is_buy: True if buy signal (False for sell)
...
def create_bearish_breakout_ea(
    ...
    """
    Create and configure Bearish Breakout EA.
    
    Args:
        symbol: Trading symbol
        timeframe: Chart timeframe
        sl_buffer_pips: Buffer to add to SL (above prev candle high)
        lot_size: Lot size
